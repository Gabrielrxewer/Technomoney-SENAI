...
export async function requireDPoP(req: any, res: any, next: any) {
  ...
  try {
    ...
    if (!token) return res.status(401).json({ message: "invalid dpop ath" });
    ...
    if (htu !== base(req)) return res.status(401).json({ message: "invalid dpop htu" });
    ...
    if (jkt !== cnf.jkt) return res.status(401).json({ message: "dpop jkt mismatch" });
